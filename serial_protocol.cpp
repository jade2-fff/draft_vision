 ...
 * @brief 串口 + CRC16 协议层实现（与下位机 USB_usart 协议完全一致）
 */
...
// ═══════════════════════════════════════════
// CRC16 查表法（多项式 0x1021，初值 0xFFFF，左移算法）
// 与下位机 USB_usart.c 完全一致
// ═══════════════════════════════════════════
...
void serial_close(int fd) { if (fd >= 0) close(fd); }

int32_t float_to_int100(float val, int32_t clamp_max) {
    int32_t v = (int32_t)std::lround(val * 100.0f);
...
void serial_send_packet(int fd, float v1, float v2, float v3) {
    if (fd < 0) return;
    int32_t a = float_to_int100(v1);
    int32_t b = float_to_int100(v2);
    int32_t c = float_to_int100(v3);
    uint8_t packet[TOTAL_PACKET_SIZE];
    std::memcpy(&packet[0], &a, 4);
    std::memcpy(&packet[4], &b, 4);
    std::memcpy(&packet[8], &c, 4);
    uint16_t crc = crc16_calculate(packet, DATA_PAYLOAD_SIZE);
...
bool serial_recv_packet(int fd, float &v1, float &v2, float &v3) {
    ...
    uint8_t tmp[256];
    ssize_t n;
    while ((n = read(fd, tmp, sizeof(tmp))) > 0) buf.insert(buf.end(), tmp, tmp + n);
    if (buf.size() > 512) buf.clear();
    bool got = false;
    while (buf.size() >= TOTAL_PACKET_SIZE) {
        uint16_t rc = buf[DATA_PAYLOAD_SIZE] | ((uint16_t)buf[DATA_PAYLOAD_SIZE + 1] << 8);
        uint16_t cc = crc16_calculate(buf.data(), DATA_PAYLOAD_SIZE);
        if (cc == rc) {
            int32_t a, b, c;
            std::memcpy(&a, &buf[0], 4);
            std::memcpy(&b, &buf[4], 4);
            std::memcpy(&c, &buf[8], 4);
            v1 = a / 100.0f; v2 = b / 100.0f; v3 = c / 100.0f;
